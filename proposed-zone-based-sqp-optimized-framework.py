...
# SQP weights for smoothing duration changes:
SQP_ALPHA = 1.0
SQP_BETA = 0.85
# Control loop update period (for SQP-based control)
CONTROL_LOOP_DT = 0.05
# SQP duration boundaries (in seconds) now set to 4.0 and 11.0
SQP_BOUNDS = [(4.0, 11.0)]
...
# Logging lists for later plotting (timestamps, effective durations, and joint positions)
duration_log = []   # Proxy for robot speed (duration of movement)
command_log = []

# Global ROS robot control node – will be initialized in main()
...
# ──────────────── Robot Control Loop (Trajectory Based with SQP) ────────────────
def robot_control_loop():
    global detection_command, robot_control_node, duration_log, command_log
    ...
    prev_duration = NOMINAL_DURATION
    ...
    while rclpy.ok():
        ...
        if current_cmd == "stop":
            ...
            # Log current state.
            duration_log.append(optimized_duration)
            command_log.append(current_cmd)
            continue

        desired_duration = SLOW_DURATION if current_cmd == "slow" else NOMINAL_DURATION

        def objective(d):
            return SQP_ALPHA * (d[0] - desired_duration)**2 + SQP_BETA * (d[0] - prev_duration)**2

        res = minimize(objective, [prev_duration], bounds=SQP_BOUNDS, method='SLSQP')
        optimized_duration = res.x[0] if res.success else desired_duration

        move_success = move_to_position_with_interrupt(robot_control_node, current_target, optimized_duration, current_cmd)
        if move_success:
            # Only when a trajectory completes normally, toggle target.
            current_target = target_position if current_target == home_position else home_position
            prev_duration = optimized_duration
        else:
            # Movement was interrupted.
            with detection_lock:
                current_mode = detection_command
            if current_mode == "stop":
                time.sleep(ROBOT_STOP_SLEEP_TIME)
            else:
                # For a mode change between normal and slow, do not toggle target;
                # update the duration smoothly and reissue command immediately.
                prev_duration = optimized_duration
        # Log current state.
        current_joints = robot_control_node.get_joint_positions()
        duration_log.append(optimized_duration)
        command_log.append(current_cmd)
        time.sleep(CONTROL_LOOP_DT)

# ──────────────── Main Function: Detection + ROS 2 + Logging ────────────────
def main():
    ...
    with open('robot_logs.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["Duration", "Command"])
        for d, c in zip(duration_log, command_log):
            writer.writerow([d, c])
